...
import type {
  ...
  PagesHelperContext,
  UniPagesConfig,
  PagesHelperDefineUniPagesConfig,
} from "./types";
export * from "./types";
...
export function UniPagesHelper(userOptions?: PagesHelperOptions): Plugin {
  ...
  return {
    ...
    transform(code, id) {
      if (!id.endsWith(ctx.options.ext) || !code.includes("</route>"))
        return null;
      /** 去掉<route></route>标签 */
...
export function defineUniPages<T = UniPagesConfig>(
  config: T | PagesHelperDefineUniPagesConfig<T>
): T {
