import type {
  ...
  UniPagesConfig,
  FilterPatternResult,
} from "@/pages-helper/types";
import type { Logger } from "lib-features";
import { createLoggerHelper, createFilter, normalizePath } from "lib-features";
...
class PagesHelper implements PagesHelperContext {
  ...
  controlPageFile: PagesHelperPagesItemConfig = {
    pkg: null,
  ...
  constructor(options: PagesHelperOptions) {
    ...
    this.pagesItems = options.pagesDirs.map((dir: string) => ({
  ...
  async loadPagesConfig() {
    ...
    const { config } = await loadConfig<UniPagesConfig>({
  ...
  async loadPagesFiles() {
    ...
    for (let i = 0; i < pagesFileList.length; i++) {
      ...
      for (let j = 0; j < filePthList.length; j++) {
        ...
        const pageBeforeResult =
          (await this.options.onPageBefore?.(this)) ?? true;

        if (pageBeforeResult === false) continue;
        this.controlPageFile.page = this.parsePageConfig();

        const pageAfterResult =
          (await this.options.onPageAfter?.(this)) ?? true;

        if (pageAfterResult === false || !this.controlPageFile.page) continue;

        this.setPagesPageItem(
          this.controlPageFile.page,
          this.controlPageFile.pkg
        );
  ...
  parsePageConfig() {
    ...
    try {
      ...
      const pageRouteContext = fileParsed.descriptor.customBlocks.find(
        (v) => v.type === "route"
      );
      const routeJson = pageRouteContext
        ? json5.parse(pageRouteContext.content)
        : {};
      routeJson.type = "page";
  ...
  getPagesConfig(isInit = false): UniPagesConfig {
    const [pages, ...subPackages] = this.pagesItems;
    let pagesConfig = merge(
      { ...PAGES_CONFIG_DEFAULT },
      this.pagesConfigFileConfig
    );
    if (!isInit) {
      const oldPagesConfig = json5.parse(
        readFileSync(this.outPagesPath, { encoding: "utf-8" })
      );
      if (this.options.writeMode === "merge") {
        pagesConfig = merge(oldPagesConfig, pagesConfig) as UniPagesConfig;
      }
    }
    ...
    pagesConfig.subPackages = subPackages
      ...
      .map((item) => ({
        ...
        pages: item.pages.map((v) => ({
          ...v,
          path: v.path.replace(new RegExp(`^${item.packageName}/`), ""),
        })),
      }));
    return pagesConfig as UniPagesConfig;
  }
  writePagesJson(isInit = false) {
    const pagesConfig = this.getPagesConfig(isInit);
  ...
  setPagesPageItem(
    ...
    if (!page || !pkg) return;
    const havePageIndex = pkg.pages.findIndex(
  ...
  getFilePkg(filePath: string): PagesHelperPagesItem | null {
    return (
      this.pagesItems.find((pkg) => filePath.startsWith(pkg.rootPath)) || null
    );
  }
  getVirtualModuleContext() {
    return JSON.stringify(this.getPagesConfig(false));
  }
  initWatcher() {
    this.watcher = chokidar.watch(this.options.pagesDirs, {
      ...
      ignored: (filePath: string) =>
        !filePath.endsWith(".vue") || this.filter(filePath),
    });
  ...
  onPageFileChange(filePath: string) {
    ...
    this.controlPageFile.pkg = this.getFilePkg(
      this.controlPageFile.currentPagePath
    );
    this.controlPageFile.page = this.parsePageConfig();
  ...
  onPageFileDelete(filePath: string) {
    ...
    const filePagePath = this.getFileRoutePath(
      this.controlPageFile.currentPagePath,
      this.controlPageFile.pkg
    );

    const havePageIndex =
      this.controlPageFile.pkg?.pages.findIndex(
        (item) => item.path === filePagePath
      ) || -1;
    if (havePageIndex >= 0) {
